...
public:
	...
	void Draw() const {
        ...
        Rectangle rocketRect = { transform.position.x - 4.f, transform.position.y - 12.f, 8.f, 24.f };
        DrawRectangleRec(rocketRect, RED);
        DrawTriangle(
            {transform.position.x, transform.position.y + 12.f},
            {transform.position.x - 4.f, transform.position.y + 18.f},
            {transform.position.x + 4.f, transform.position.y + 18.f},
            WHITE
        );
    }
	...
	float speed)
{
	...
	if (wt == WeaponType::LASER) {
        ...
        return Projectile(pos, Vector2Scale(vel,2.0f), 25, wt); 
    }
	else {
...
public:
	...
	void Run() {
		...
		float shotTimer = 0.f;
		zielonybuch = LoadTexture("zielonybuch.png");
		while (!WindowShouldClose()) {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						 if (pit->GetType() == WeaponType::ROCKET) {
					explosions.push_back({ (*ait)->GetPosition(), 0.f });
				}
				score+=100;
			...
			// Update explosions
				for (auto it = explosions.begin(); it != explosions.end(); ) {
					it->timer += dt;
					if (it->timer > Explosion::DURATION) {
			...
			// Render everything
			{
				...
				DrawText(TextFormat("Weapon: %s", weaponName),
					10, 40, 20, BLUE);
				DrawText(TextFormat("Score: %d", score), 10, 70, 20, PURPLE);
				...
				float alpha = 1.0f - (exp.timer / Explosion::DURATION);
				Color tint = WHITE;
				tint.a = static_cast<unsigned char>(255 * alpha);
				float scale = 0.3f; // Adjust this value as needed
				Vector2 pos = {
					exp.position.x - (zielonybuch.width * scale) / 2,
					exp.position.y - (zielonybuch.height * scale) / 2
				};
				DrawTextureEx(zielonybuch, pos, 0.0f, scale, tint);
				}
